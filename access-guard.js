...
function injectStyles(){
  ...
  style.textContent = `
    ...
    /* Fixed user controls: anchored to the visual viewport and safe-area aware on iPhone. */
    #msiUserBar{position:fixed;right:16px;right:max(16px,env(safe-area-inset-right));bottom:16px;bottom:max(16px,env(safe-area-inset-bottom));z-index:99980;background:#111827;border:1px solid #374151;border-radius:10px;padding:9px 11px;color:#d1d5db;font:11px -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;display:flex;gap:8px;align-items:center;box-shadow:0 8px 25px rgba(0,0,0,.25);margin:0;transform:none!important;transition:none!important;animation:none!important;float:none!important;will-change:auto;contain:layout paint;}
    #msiUserBar button{border:1px solid #4b5563;border-radius:7px;background:#1f2937;color:#fff;padding:6px 8px;font-size:10px;font-weight:800;cursor:pointer;transform:none!important;transition:none!important;animation:none!important;}
    @media(max-width:600px){#msiUserBar{right:max(10px,env(safe-area-inset-right));bottom:max(10px,env(safe-area-inset-bottom));padding:7px 9px;font-size:10px;gap:6px}#msiUserBar button{padding:6px 7px;font-size:10px}}
  `;
