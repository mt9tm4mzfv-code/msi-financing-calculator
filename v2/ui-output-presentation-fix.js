...
(function(){
  ...
  function normalize(n){
    ...
    if(!results || results.dataset.msiNormalizing==='1')return;
    ...
    try{
      ...
      requestAnimationFrame(()=>{delete results.dataset.msiNormalizing;});
    }
  }
  ...
  function init(){
    [1,2,3].forEach(n=>{
      const results=document.getElementById(`c${n}_results`);
      if(!results)return;
      const observer=new MutationObserver(()=>{if(results.dataset.msiNormalizing!=='1')schedule(n);});
