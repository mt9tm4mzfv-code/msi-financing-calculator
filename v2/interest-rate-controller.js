...
(function(){
  ...
  function interceptSimpleButtons(){document.addEventListener('click',function(event){
    ...
    if(copy){const simple=copy.closest('.simple-results'),m=simple?.id?.match(/^c([123])_simple_results$/);if(!m)return;event.preventDefault();event.stopImmediatePropagation();writeClipboard(simpleCopyText(Number(m[1])),'Simple computation copied.')}
  },true)}
  function writeClipboard(text,msg){if(navigator.clipboard?.writeText)navigator.clipboard.writeText(text).then(()=>showToast(msg)).catch(()=>fallbackCopy(text,msg));else fallbackCopy(text,msg)}
