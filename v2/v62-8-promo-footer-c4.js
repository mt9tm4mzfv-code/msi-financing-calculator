...
(function(){
  ...
  function moveBDP(n){
    const b=document.getElementById(`c${n}_bdp`),op=document.getElementById(`c${n}_opdp`);if(!b||!op||b.dataset.v628Moved)return;
    const oldGrid=b.closest('.grid');const oldLabel=oldGrid?.querySelector(`label[for="c${n}_bdp"]`);const opCell=op.parentElement;
    if(oldLabel&&opCell){const cell=document.createElement('div');cell.className='promo-bdp-cell';oldLabel.textContent='Promo DP Percentage (%)';cell.appendChild(oldLabel);cell.appendChild(b);opCell.parentElement.insertBefore(cell,opCell.nextSibling);b.dataset.v628Moved='1';}
    b.min='0';b.max='100';b.step='0.01';b.value=b.value||'20';b.setAttribute('aria-label','Promo DP Percentage (%)');
  }
  ...
  function init(){[1,2,3].forEach(moveBDP);installC4();wrapCopies();interceptSimpleCopy();setTimeout(()=>{[1,2,3].forEach(moveBDP);installC4();wrapCopies()},100);setTimeout(()=>{[1,2,3].forEach(moveBDP);installC4();wrapCopies()},500)}
